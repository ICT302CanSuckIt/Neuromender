...
public class MainMenuController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        HideCreditSlide();
        OptionsMenu.transform.FindChild("ShowPrevTeams").gameObject.SetActive(false);
        OptionsMenu.transform.FindChild("RtnOptions").gameObject.SetActive(false);
    ...
    void FormatMenu(MENU theMenu)
    {
        switch (theMenu)
        {
            case MENU.TASK:
                ...
                TaskMenu.transform.FindChild("ElbowRaiseButton").gameObject.SetActive(login.config.allowWingman);

                break;
            case MENU.ARM_EXTEND:

                // Check if the Targets functionality has been enabled.
                ArmExtensionMenu.transform.FindChild("TargetsButton").gameObject.SetActive(login.config.allowTargets);

                // Check if the Rowing functionality has been enabled.
                ArmExtensionMenu.transform.FindChild("RowingButton").gameObject.SetActive(login.config.allowRowing);

                // Check if the Cycling functionality has been enabled.
                ArmExtensionMenu.transform.FindChild("CyclingButton").gameObject.SetActive(login.config.allowCycling);

                // Disable access to the whole arm extension menu if neither 'Targets' OR 'Rowing' games are available to the survivor.
                TaskMenu.transform.FindChild("ArmExtensionButton").gameObject.SetActive(!(!login.config.allowTargets && !login.config.allowRowing && !login.config.allowCycling));
    ...
    public void ShowCreditSlide()
    {
        ...
        OptionsMenu.transform.FindChild("ShowPrevTeams").gameObject.SetActive(true);
        OptionsMenu.transform.FindChild("RtnOptions").gameObject.SetActive(true);
        OptionsMenu.transform.FindChild("Show Credits").gameObject.SetActive(false);
        OptionsMenu.transform.FindChild("ToggleDebug").gameObject.SetActive(false);
        OptionsMenu.transform.FindChild("Return").gameObject.SetActive(false);
    ...
    public void HideCreditSlide()
    {
        ...
        OptionsMenu.transform.FindChild("ShowPrevTeams").gameObject.SetActive(false);
        OptionsMenu.transform.FindChild("RtnOptions").gameObject.SetActive(false);
        OptionsMenu.transform.FindChild("Show Credits").gameObject.SetActive(true);
        OptionsMenu.transform.FindChild("ToggleDebug").gameObject.SetActive(true);
        OptionsMenu.transform.FindChild("Return").gameObject.SetActive(true);
    }
    ...
    {
        SceneManager.LoadScene("easyroute-backup");
    }
