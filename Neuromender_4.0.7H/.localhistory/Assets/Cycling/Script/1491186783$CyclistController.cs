...
public class CyclistController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (CarIsFinished == false)
        {
            ...
            if (colRPM.GetComponent<colliderRPM>().counter > 0)
            {
                ...
                else
                {
                    ...
                    transform.rotation = Quaternion.Slerp(transform.rotation, wantedRotation, Time.deltaTime * 2.5f);
                    ...
                    if (colRPM.GetComponent<colliderRPM>().counter > NewCounter || (Input.GetKey(KeyCode.UpArrow)))
                    {
                        velocity = (path.nodes[targetIndex] - path.nodes[targetIndex - 1]).normalized * 3f;
                        NewCounter++;
                    }
    ...
    private double Distanceupdate(int k, int nodes)
    {
        double dist = 0;
