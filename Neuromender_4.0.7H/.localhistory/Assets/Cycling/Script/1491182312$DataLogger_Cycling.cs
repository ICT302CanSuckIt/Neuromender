...
public class DataLogger_Cycling : DataLogger_Base
{
        ...
        if (user != "" && loggedGame != "")
        {
            ...
            using (STREAM = new StreamWriter(dataPath))
            {
                ...
                for (int i = 0; i < DiamondNo; i++)
                {
                    ...
                    STREAM.Write(dat.DiamondNum + "," + dat.score + "," + dat.RotationNum + "," + dat.timeInterval + "," + ",,,,,,,,,,,,,,,,");
    ...
    public override void PushData()
    {
        ...
        for (int i = 0; i < DiamondNo; i++)
        {
            ...
            fields += ",TotalTime"; //not fucntioning for now
            values += 0 + ",";

            fields += ",Speed"; //not fucntioning for now
            values += 0 + ",";
...
public class CyclingDataMass : DataMass_Base
{
    ...
    public CyclingDataMass(int DiaNum, int points, int Rotation, float Interval)
    {
        DiamondNum = DiaNum;
        score = points;
