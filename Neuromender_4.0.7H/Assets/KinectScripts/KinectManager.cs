...
public class KinectManager : MonoBehaviour
{
    ...
    public bool hintHeightAngle = true;
    ...
    public float minUserDistance = 0.2f;
    ...
    private void ProcessBodyFrameData()
    {
        ...
        for (int i = 0; i < sensorData.bodyCount; i++)
        {
            ...
            float maxUserDistance;

            DBcons = GameObject.Find("DatabaseController");
            if (DBcons != null)
            {
                maxUserDistance = (DBcons.GetComponent<LoginControl>().config.sensorDistance / 1000);
            }
            else
                maxUserDistance = 1.5f;
           ...
           //maxUserDistance = 1.5f;
            ...
            /*
            if (DBcons.GetComponent<LoginControl>().LoggedIn == false)
            {
                maxUserDistance = 1.5f;
            }
            else
   ...
   //////////////////////////////     //     Debug.Log(string.Format("maxUserDistance k = " + maxUserDistance));
               ...
               (maxUserDistance <= 0f || Mathf.Abs(bodyData.position.z) <= maxUserDistance))
            {
                ...
                //Debug.Log(string.Format("distance one " + Mathf.Abs(bodyData.position.z)));

                /*if ( Mathf.Abs(bodyData.position.z) > maxUserDistance)
                {
                    break;
                }*/
                ...
                if (liPrimaryUserId == 0)
                {
                    ...
                    /*if (Mathf.Abs(bodyData.position.z) > 1.5f)
                    {
                        bClosestUser = false;
                        break;
                    }*/
                    ...
                    if (detectClosestUser)
                    {
                        for (int j = 0; j < sensorData.bodyCount; j++)
                        {
                            if (j != i)
                            {
                              ...
                              /////////////////////////      Debug.Log(string.Format("distance of bosy " + Mathf.Abs(bodyPos.z) + "and " + Mathf.Abs(bodyDataOther.position.z)));
                       ...
                       ////////////////////////         Debug.Log(string.Format("bodyDataOther"));

                            }
                ////////////////////////            Debug.Log(string.Format("sensorData.bodyCount" + sensorData.bodyCount));

                        }

           /////////////////////////             Debug.Log(string.Format("detectClosestUser"));
